...
const DATA2 = [
  ...
  },
  {
    ...
    top_reply: 'apabila nun mati atau tanwin bertemu dengan alif, ha, ha, kho, ain, ghoin maka hukum tazwidnya adalah izhar. apabila nun mati atau tanwin bertemu dengan nun. mim, wau, ya maka hukun tazwidnya adalah idgham bighunnah'
  },
  {
    id: '3',
    nama: 'Ujang Asep',
...
const HeaderItem = () => (
    ...
    <View className='flex-1  h-16 border-abumuda justify-center ml-4'>
      <Text className='font-poppins-semi-bold text-xl text-hitam'>Percakapan</Text>
    </View>
    <View className='flex-row items-center'>
    <Text className='font-poppins-medium text-sm text-hijau mr-2'>Terbaru</Text>
    <SvgArrowBot/>
  ...
  <View className='absolute bottom-0 bg-white flex flex-1 flex-row stretch p-5'>
        <TouchableOpacity style={{flex: 1}}>
            <View className='h-14 bg-hijau rounded-sm items-center justify-center'>
                <Text className='font-poppins-semi-bold text-base text-white'>TAMBAH DISKUSI</Text>
            </View>
        </TouchableOpacity>
  </View>
);
const CardItem2 = ({ item } : { item: Item2}) => (
  <View className='flex-row py-4'>
    <View>
    <Image source={require('../../assets/profile.png')} className='h-9 w-9 rounded' />
    </View>
    <View className='flex-1 ml-5'>
    <Text className='font-poppins-medium text-base text-hitam'>{item.nama}</Text>
    <Text className='font-poppins-regular text-sm text-hitam mt-2'>{item.text}</Text>
    <View className='flex-row mt-6'>
        <View className='flex-1 flex-row items-center'>
        <SvgThumb1/>
        <Text className='font-poppins-medium text-sm text-hitam ml-2'>{item.like} Suka</Text>
        </View>
        <View className='flex-1 flex-row items-center'>
        <SvgReply/>
        <Text className='font-poppins-medium text-sm text-hitam ml-2'>{item.reply} Balasan</Text>
        </View>
    </View>
    </View>
  </View>
