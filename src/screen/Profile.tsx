...
const ButtonItem = ({item}: {item: Item}) => (
    <View className="flex-row h-32 rounded mr-4 my-4" key={item.id}>
        <Text className="font-poppins-medium text-base text-hitam ml-3">
            {item.title}
        </Text>
    </View>
);
...
const CardItem = ({item}: {item: Item2}) => (
    <View className="flex w-60 h-60 border border-abumuda rounded mr-4">
        <Image
            ...
            className="w-60 h-28"
        />
        <Text className="font-poppins-medium text-xs text-hijau ml-4 mt-3">
            {item.category}
        </Text>
        <Text className="font-poppins-medium text-base text-hitam ml-4 mt-2">
            {item.title}
        </Text>
        ...
        <View className="flex-1 flex-row items-end mx-4 mb-2">
            <View className="flex-1 flex-row">
                ...
                <Text className="font-poppins-regular text-sm text-abutua ml-2 mr-auto">
                    {item.count} Materi
                </Text>
            </View>
...
const CardItem2 = ({item}: {item: Item2}) => (
    <View className="flex-row h-32 rounded mr-4 my-4" key={item.id}>
        ...
        <View className="flex-1 h-32">
            <Text className="font-poppins-medium text-xs text-hijau ml-4">
                {item.category}
            </Text>
            <Text className="font-poppins-medium text-base text-hitam ml-4 mt-2">
                {item.title}
    ...
    return (
        <SafeAreaView className="flex-1">
            <ScrollView
                ...
                <Text className="font-poppins-medium text-xl text-hitam ml-36 mt-6">
                    {auth.authData?.user.first_name}{' '}
                    {auth.authData?.user.last_name}
                </Text>
                <Text className="font-poppins-regular text-sm text-hitam ml-36">
                    {auth.authData?.user.username}
                </Text>
                <Text className="font-poppins-semi-bold text-lg p-5 mt-4 text-hitam">
                    Pesan & Kesan
                </Text>
                <Text className="font-poppins-regular text-sm px-5 pb-8 text-hitam">
                    {auth.authData?.user.profile.bio}
                </Text>
                ...
                <View className="px-5 flex-row items-center">
                    <View className="flex-1">
                        <Text className="font-poppins-semi-bold text-lg text-hitam mr-auto">
                            Prestasi
                        </Text>
                    </View>
                    <TouchableOpacity
                        onPress={() => navigation.navigate('Prestasi')}>
                        <Text className="font-poppins-medium text-sm text-hijau text-end">
                            Lihat Semua
                        </Text>
                    </TouchableOpacity>
                </View>
                ...
                <Text className="font-poppins-semi-bold text-lg p-5 text-hitam">
                    Sedang Mempelajari
                </Text>
