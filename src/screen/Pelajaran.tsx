...
const ButtonItem = ({item}: {item: Kategori}) => (
    <TouchableOpacity>
        <View className="flex w-24 h-24 border border-abumuda rounded items-center justify-center mr-4">
            <Text className="font-poppins-medium text-sm text-hitam">
                {item.name}
            </Text>
        </View>
    </TouchableOpacity>
...
const CardItem2 = ({item, navigation}: {item: Pelajaran; navigation: any}) => (
    <View className="flex-row h-32 rounded mr-4 my-4" key={item.pk}>
        <TouchableOpacity
            onPress={() =>
                navigation.navigate('DetailPelajaran', {id: item.pk})
            }>
            <Image
                source={{
                    uri: item.image ? item.image : image.default_pelajaran,
                }}
        ...
        <View className="flex-1 h-32">
            ...
            <View className="flex-1 flex-row ml-4 items-end">
                ...
                <View className="flex-1 flex-row">
                    ...
                    />
                    <Text className="font-poppins-regular text-sm text-abutua ml-2">
                        {item.average_star ? item.average_star : 0}
                    </Text>
                </View>
            </View>
        </View>
...
const Pelajaran = ({navigation}: {navigation: any}) => {
    ...
    return (
        <SafeAreaView className="flex-1">
            <ScrollView
                className="pt-5 pb-20 bg-white"
                contentContainerStyle={{flexGrow: 1}}>
                <TextInput
                    placeholder="Cari Pelajaran"
                    className="border-abusedang border rounded h-14 font-poppins-regular text-sm px-5 mx-5"
                ...
                <View className="flex-row items-center mt-5 mx-5">
                    <Text className="font-poppins-semi-bold text-lg text-hitam mr-auto flex-1">
                        {list.length} Pelajaran Ditemukan
                    </Text>
                </View>
                <View className="p-5">
