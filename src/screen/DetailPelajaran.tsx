...
const ButtonItem = ({item}: {item: Item}) => (
    <View className="flex-row rounded mr-4 my-4" key={item.pk}>
        <Text className="font-poppins-semi-bold text-lg text-hitam ml-3">
            {item.title}
        </Text>
    </View>
);
    ...
    item.tipe == 'MT' ? (
        <View
            ...
            <View className="h-14 w-14 rounded-sm border-2 border-abuhijau items-center justify-center">
                <Text className="font-poppins-medium text-lg text-hijau">
                    #{item.pk}
                </Text>
            </View>
            <View className="flex-1">
                <Text className="font-poppins-medium text-base text-hitam ml-4">
                    {item.name}
                </Text>
                <Text className="font-poppins-regular text-sm text-hitam ml-4">
                    {item.menit} menit
                </Text>
            </View>
            <View className="w-14 h-14 justify-center items-center">
                <TouchableOpacity
                    onPress={() =>
                        navigation.navigate('SubMateri', {id: item.pk})
                    }>
        ...
        <SafeAreaView className="flex-1">
            <ScrollView
                ...
                <View className="absolute h-36  top-36 left-7">
                    ...
                    <View>
                        <Text className="font-poppins-semi-bold text-lg text-white pt-3">
                            {data.name}
                        </Text>
                    </View>
                    <View className="flex-1 flex-row items-end">
                        <View className="flex-1 flex-row">
                            ...
                            <Text className="font-poppins-regular text-sm text-white mx-2">
                                {data.submateri ? data.submateri?.length : 0}{' '}
                                Materi
                            </Text>
                        </View>
                        <View className="flex-row">
                            ...
                            <Text className="font-poppins-regular text-sm text-white ml-2">
                                {data.sum_menit ? data.sum_menit : 0} menit
                            </Text>
                        </View>
                    </View>
                </View>
                        ...
                        <View className="flex-1 justify-center">
                            <View className="flex-row">
                                <View className="h-12 w-12 bg-white rounded-full items-center justify-center">
                                    <View className="h-10 w-10 bg-abusedang rounded-full"></View>
                                </View>
                            ...
                            <Text className="font-poppins-regular text-sm text-abutua mt-3 self-center">
                                {data.total_pelajar} Siswa Belajar
                            </Text>
                        </View>
                        <View className="flex-1 justify-center items-end">
                            <Text className="font-poppins-semi-bold text-xl text-hitam ml-2">
                                {data.average_star ? data.average_star : 0}
                            </Text>
                            ...
                            <TouchableOpacity
                                onPress={() =>
                                    navigation.navigate('Percakapan')
                                }>
                                <Text className="font-poppins-regular text-sm text-abutua mt-2">
                                    {data.total_diskusi} Percakapan
                                </Text>
                            </TouchableOpacity>
                        </View>
                    </View>
